 ...
 */
export const getCellGroupIndex = ({
	rowName,
	columnName,
	options,
}: {
 ...
 */
export const quantifyInputKey = (
	inputKey: string,
	options: PuzzleOption[]
): number => {
	const size: number = options.reduce(
		(size, option) => Math.max(size, option.values.length),
		0
	);
	const { rowName, rowValue, columnName, columnValue } =
		parseInputKey(inputKey);
 ...
 */
export const getEmptyInputForOptions = (options: PuzzleOption[]) => {
	if (options.length === 0) {
		return [];
	}
	...
	return firstOption.values.map((val: string) =>
		Object.assign({}, emptyRow, { [`${firstOption.name}`]: val })
	);
};
 ...
 */
export const normalizePuzzleInput = ({
	input,
	options,
}: {
